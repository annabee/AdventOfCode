...
def get_wire_coordinates_with_steps(x, y, directions):
    ...
    for direction in directions:
        ...
        while distance:
            ...
            y += y_delta
            steps += 1
            wire_coordinates.update({(x, y): steps})
            distance -= 1
...
def main():
    first_wire_coordinates_with_steps = get_wire_coordinates_with_steps(0, 0, first_wire.split(","))
    second_wire_coordinates_with_steps = get_wire_coordinates_with_steps(0, 0, second_wire.split(","))

    crossed_wires = set(first_wire_coordinates_with_steps.keys()) \
        .intersection(set(second_wire_coordinates_with_steps.keys()))

    manhattan_dists = [abs(x) + abs(y) for (x, y) in crossed_wires]

    print(crossed_wires)
    print(min(manhattan_dists))

    steps_to_cross = [
        first_wire_coordinates_with_steps[(x, y)] + second_wire_coordinates_with_steps[(x, y)] for (x, y) in
        crossed_wires
    ]

    print(min(steps_to_cross))
